...
public func makeTools(ipc: SidekickIPCClient) -> [Tool] {
    ...
    Tool(
        ...
        description: "Read a pane's output, once. Never poll in a loop: block on sidekick_wait_agent_status (a worker finishing) or sidekick_wait_output (a specific line), then take ONE bounded read. source=visible (default; the screen, bounded and cheap) is the right source for a progress check. source=recent is a transcript of the pane's scrollback — use it when you genuinely need history. A source=recent read returns a `cursor`; pass it back as `since` to get only the output appended since, so a follow-up read is a delta, not the whole buffer again. On a stale cursor (evicted or a restarted shell) the reply is `truncated: true` with a full re-read, never an error. Set json=true for structured per-command records {command, exit_code, duration, output} built from shell-integration marks — easier to reason over than raw text. In visible reads, text wrapped in ⟦suggested, not typed: …⟧ is an unaccepted autosuggest/placeholder rendered at the cursor — the user did NOT type it; it becomes real input only if accepted (typically via the right-arrow key).",
        inputSchema: object([
            "pane_id": paneIDProperty,
            "source": ["type": "string", "enum": ["visible", "recent"], "description": "visible (default; the screen — prefer it for progress checks) or recent (scrollback transcript, for history)."],
